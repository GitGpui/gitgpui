...
- ✅ Headless-build GUI validation is now explicit: `gitgpui-app difftool --gui` and `gitgpui-app mergetool --gui` fail fast with exit `2` and actionable guidance when built without `ui-gpui`, preventing silent CLI fallback. Covered by standalone integration tests gated with `#[cfg(not(feature = "ui-gpui"))]`.
...
- ✅ Explicit missing-tool command parity is now covered in git-invoked E2E tests: `git mergetool --tool absent` and `git difftool --tool absent` both fail with actionable `cmd not set for tool` diagnostics.
...
- ✅ Setup placeholder-quoting parity is now explicitly gated: all generated headless and GUI setup commands must keep placeholders in quoted form only (`"$BASE"`, `"$LOCAL"`, `"$REMOTE"`, `"$MERGED"`). Enforced in both `setup_mode.rs` unit tests and `standalone_tool_mode_integration.rs` setup E2E tests.
...
- ✅ Deleted-vs-modified submodule orientation parity is now explicit for the reverse merge direction as well: path-targeted `git mergetool submod` assertions now verify that when local is modified and remote is deleted, `r` keeps deletion and `l` keeps the modified submodule gitlink, completing the t7610 matrix.
...
- ✅ Phase 4A deleted-vs-modified submodule reverse-orientation hardening: additional `l`/`r` assertions now cover the opposite merge direction (local modified, remote deleted), validating both outcomes from the full `t7610` matrix.
