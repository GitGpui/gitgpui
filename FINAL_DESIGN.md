...
- ✅ Scanned all project code for `TODO`/`FIXME`/`todo!()`/`unimplemented!()` — none found in production code (only in vendor code and test mock stubs).
- ✅ Deep subagent verification of every design document item against actual codebase — every item confirmed implemented and tested.
...
Conclusion: All components from both design documents remain fully implemented. Iteration 50 performed a clean verification pass confirming 1206 tests passing, zero clippy warnings, and no production-code gaps.
...
Performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Full workspace test suite: `cargo test --workspace --no-default-features --features gix` — **1204 passed, 0 failed, 5 ignored**.
- ✅ Clippy clean: `cargo clippy --workspace --no-default-features --features gix -- -D warnings` — **0 warnings**.
- ✅ Scanned all project code for `TODO`/`FIXME`/`todo!()`/`unimplemented!()` — none found in production code (only in vendor code).
...
Conclusion: All components from both design documents remain fully implemented. Iteration 49 performed a clean verification pass confirming 1204 tests passing, zero clippy warnings, and no production-code gaps.
...
Performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Full workspace test suite: `cargo test --workspace --no-default-features --features gix` — **1202 passed, 0 failed, 5 ignored**.
- ✅ Clippy clean: `cargo clippy --workspace --no-default-features --features gix -- -D warnings` — **0 warnings**.
- ✅ Deep subagent verification of all design document items against actual codebase — every item confirmed implemented and tested.
...
Conclusion: All components from both design documents remain fully implemented. Iteration 48 performed a clean verification pass confirming 1202 tests passing, zero clippy warnings, and no production-code gaps.

### Progress Snapshot (Iteration 47, Mergetool Cmd-vs-Path Precedence Hardening — March 2, 2026)

Performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Implemented missing regression coverage for external-mergetool config precedence in [`crates/gitgpui-git-gix/tests/status_integration.rs`](/home/sampo/gitgpui/crates/gitgpui-git-gix/tests/status_integration.rs):
  - Added `launch_mergetool_prefers_custom_cmd_over_tool_path_override`.
  - Verifies that when both `mergetool.<tool>.cmd` and `mergetool.<tool>.path` are configured, GitGpui executes `cmd` and does not run the path executable.
  - Confirms merged output comes from `cmd` and asserts no side-effect file from the path tool invocation.
- ✅ Validation:
  - `cargo test -p gitgpui-git-gix launch_mergetool_prefers_custom_cmd_over_tool_path_override`
  - `cargo test -p gitgpui-git-gix launch_mergetool_uses_tool_path_override_without_custom_cmd`
  - `cargo test -p gitgpui-git-gix launch_mergetool_custom_cmd_supports_braced_env_variables`
...
- ✅ Behavior matrix: all 10 required scenarios covered by automated tests.
- ✅ Test strategy: all three sections (A: Git scenarios, B: existing test extensions, C: fixture harness) complete.
  - Hardening this iteration: explicit regression lock for `mergetool.<tool>.cmd` precedence over `mergetool.<tool>.path`.
- ✅ Rollout plan: all three phases (MVP, compat parity hardening, regression suite) complete.
- ✅ Acceptance criteria: all 5 criteria met.
- 🔧 Partially implemented components: none.
- ⬜ Not-yet-started components: none.
...
Conclusion: All components from both design documents remain fully implemented. Iteration 47 adds explicit regression protection for custom mergetool command precedence when path overrides are also configured.

### Progress Snapshot (Iteration 47, Independent Completion Verification — March 2, 2026)

Performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Full workspace test suite: `cargo test --workspace --no-default-features --features gix` — **1201 passed, 0 failed, 5 ignored**.
- ✅ Clippy clean: `cargo clippy --workspace --no-default-features --features gix -- -D warnings` — **0 warnings**.
- ✅ Scanned all project code for `TODO`/`FIXME`/`todo!()`/`unimplemented!()` — none found in production code (only in vendor code and expected test mock stubs).
- ✅ Deep verification of all design document items against actual codebase via subagent exploration — every item confirmed implemented and tested.
...
Conclusion: All components from both design documents remain fully implemented. Iteration 47 performed a clean verification pass confirming 1201 tests passing, zero clippy warnings, and no production-code gaps.
...
Performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Full workspace test suite: `cargo test --workspace --no-default-features --features gix` — **1200 passed, 0 failed, 5 ignored**.
- ✅ Clippy clean: `cargo clippy --workspace --no-default-features --features gix -- -D warnings` — **0 warnings**.
- ✅ Scanned all project code for `TODO`/`FIXME`/`todo!()`/`unimplemented!()` — none found in production code (only in vendor code and expected test mock stubs).
...
Conclusion: All components from both design documents remain fully implemented. Iteration 46 performed a clean verification pass confirming 1200 tests passing, zero clippy warnings, and no production-code gaps.

### Progress Snapshot (Iteration 45, Phase 3C Zero-Limit Extraction Validation Hardening — March 2, 2026)

Performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Hardened extraction runtime validation in [`crates/gitgpui-app/src/extract_fixtures_mode.rs`](/home/sampo/gitgpui/crates/gitgpui-app/src/extract_fixtures_mode.rs):
  - Added direct execution-layer checks rejecting `max_merges == 0`.
  - Added direct execution-layer checks rejecting `max_files_per_merge == 0`.
- ✅ Added regression coverage:
  - Unit tests: `run_extract_merge_fixtures_rejects_zero_max_merges`, `run_extract_merge_fixtures_rejects_zero_max_files_per_merge`.
  - E2E test: `extract_merge_fixtures_e2e_rejects_zero_max_files_per_merge`.
- ✅ Validation: `cargo test -p gitgpui-app --test extract_merge_fixtures_integration --no-default-features --features gix` — **4 passed, 0 failed**.
- ✅ Validation: `cargo test -p gitgpui-app --no-default-features --features gix` — **355 passed, 0 failed**.
...
- ✅ Phase 2A–2C: KDiff3-style fixture harness — 18 tests + 9 seed fixtures.
- ✅ Phase 3A–3C: Permutation corpus (243 sampled + 161K on-demand) + real-world merge extraction.
  - Hardening this iteration: extraction runtime now rejects zero extraction limits even when invoked outside CLI parsing, and E2E coverage now includes `--max-files-per-merge=0`.
- ✅ Phase 4A: Mergetool E2E — 65 tests.
- ✅ Phase 4B: Difftool E2E — 32 tests.
- ✅ Phase 5A–5C: Meld-derived algorithm tests — 32 tests.
- 🔧 Partially implemented components: none.
...
Conclusion: All components from both design documents remain fully implemented. This iteration hardened Phase 3C extraction command-surface robustness by adding execution-layer zero-limit validation and matching integration coverage.

### Progress Snapshot (Iteration 45, Independent Completion Verification — March 2, 2026)

Performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Full workspace test suite: `cargo test --workspace --no-default-features --features gix` — **1197 passed, 0 failed, 5 ignored**.
- ✅ Clippy clean: `cargo clippy --workspace --no-default-features --features gix -- -D warnings` — **0 warnings**.
- ✅ Scanned all project code for `TODO`/`FIXME`/`todo!()`/`unimplemented!()` — none found in production code (only in test mock stubs in `gitgpui-state/src/store/tests*.rs`, which are expected unused trait method stubs).
...
Conclusion: All components from both design documents remain fully implemented. Iteration 45 performed a clean verification pass confirming 1197 tests passing, zero clippy warnings, and no production-code gaps.
...
Performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Full workspace test suite: `cargo test --workspace --no-default-features --features gix` — **897 passed, 0 failed, 3 ignored**.
- ✅ Clippy clean: `cargo clippy --workspace --no-default-features --features gix -- -D warnings` — **0 warnings**.
...
- ✅ Independent agent-based audit of CLI implementation (cli.rs, difftool_mode.rs, mergetool_mode.rs, setup_mode.rs) — all subcommands, feature gates, and error paths verified complete.
- ✅ Independent agent-based audit of behavior matrix test coverage — all 10 required scenarios confirmed with 65+ dedicated test functions across integration and unit suites.
...
Conclusion: All components from both design documents remain fully implemented. This iteration performed a comprehensive independent verification: full test suite, clippy, code scan for unfinished markers, and agent-based audit of both implementation and test coverage. No gaps found.
...
Performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Identified a remaining Phase 3C extraction gap in `crates/gitgpui-core/src/merge_extraction.rs`:
  - extraction skipped files when any side blob was absent (`git show <sha>:<path>` missing), which dropped add/add and modify/delete-style cases from real-world corpus generation.
- ✅ Implemented missing-side extraction support:
  - `extract_merge_cases()` now materializes missing base/parent blobs as empty content instead of skipping the case.
  - this preserves non-trivial missing-side conflict shapes while still filtering trivial samples (`base == contrib1`, `base == contrib2`, `contrib1 == contrib2`).
- ✅ Added regression coverage:
  - new test `extracts_cases_with_missing_base_or_parent_as_empty_text` builds a real merge history with both add/add and modify/delete conflicts and verifies extracted cases include both paths with correct empty-side materialization.
- ✅ Validation: `cargo test -p gitgpui-core merge_extraction -- --nocapture` (**12 passed, 0 failed**).
- ✅ Validation: `cargo test --workspace --no-default-features --features gix` (**all passed, 0 failed, 5 ignored**).
...
- ✅ Phase 2A–2C: KDiff3-style fixture harness — 18 tests + 9 seed fixtures.
- ✅ Phase 3A–3C: Permutation corpus (243 sampled + 161K on-demand) + real-world merge extraction.
  - Hardening this iteration: extraction now includes add/add and modify/delete style cases by treating missing side blobs as empty text, increasing real-world corpus diversity.
- ✅ Phase 4A: Mergetool E2E — 65 tests.
- ✅ Phase 4B: Difftool E2E — 32 tests.
- ✅ Phase 5A–5C: Meld-derived algorithm tests — 32 tests.
- 🔧 Partially implemented components: none.
...
Conclusion: All components from both design documents remain fully implemented. This iteration hardened Phase 3C corpus extraction so missing-side merge cases are no longer silently skipped.
...
Performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Identified a remaining Phase 2 fixture-harness parsing gap in `crates/gitgpui-core/tests/merge_fixture_harness.rs`:
  - fixture discovery matched the first `"_base."` token in filenames, which could mis-parse valid `*_base.*` fixtures when the prefix itself contained `"_base"`.
  - `*_actual_result.*` artifact naming only preserved the last extension segment (e.g. `.txt`), not full multi-dot suffixes from `*_base.*`.
- ✅ Hardened fixture parsing and artifact naming:
  - added `parse_base_fixture_filename()` using suffix-oriented parsing (`rfind("_base.")`) for correct `*_base.*` handling.
  - `discover_fixtures()` now derives companion paths (`_contrib1`, `_contrib2`, `_expected_result`) from the parsed suffix tuple.
  - `actual_result_path()` now preserves the full extension suffix from the base fixture name (e.g. `.merge.fixture.txt`).
- ✅ Added regression tests:
  - `discover_fixtures_uses_last_base_marker_in_filename`
  - `actual_result_path_preserves_multi_dot_extension_suffix`
- ✅ Validation: `cargo test -p gitgpui-core --test merge_fixture_harness -- --nocapture` (**18 passed, 0 failed**).
...
- ✅ Behavior matrix: all 10 required scenarios covered by automated tests.
- ✅ Test strategy: all three sections (A: Git scenarios, B: existing test extensions, C: fixture harness) complete.
  - Hardening this iteration: fixture harness `*_base.*` discovery is now suffix-correct for embedded `"_base"` prefixes and multi-dot fixture extensions.
- ✅ Rollout plan: all three phases (MVP, compat parity hardening, regression suite) complete.
- ✅ Acceptance criteria: all 5 criteria met.
- 🔧 Partially implemented components: none.
- ⬜ Not-yet-started components: none.
...
- ✅ Phase 1C: Conflict label formatting — 5 tests.
- ✅ Phase 2A–2C: KDiff3-style fixture harness — 18 tests + 9 seed fixtures.
  - Hardening this iteration: fixture auto-discovery now correctly handles prefixes containing `"_base"` and preserves full `*_base.*` extension suffixes when writing `*_actual_result.*` artifacts.
- ✅ Phase 3A–3C: Permutation corpus (243 sampled + 161K on-demand) + real-world merge extraction.
- ✅ Phase 4A: Mergetool E2E — 65 tests.
- ✅ Phase 4B: Difftool E2E — 32 tests.
- ✅ Phase 5A–5C: Meld-derived algorithm tests — 32 tests.
...
Conclusion: All components from both design documents remain fully implemented. This iteration hardened Phase 2 fixture filename handling so `*_base.*` discovery and `*_actual_result.*` artifact naming remain correct for embedded `"_base"` prefixes and multi-dot extensions.
...
Performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Strengthened CRLF conflict marker unit test in `mergetool_mode.rs`:
  - `crlf_conflict_markers_match_input_line_endings` now asserts that conflict markers (`<<<<<<<`, `=======`, `>>>>>>>`) are terminated with `\r\n` and that all line endings in the output are consistently CRLF.
  - Previously only checked for marker presence without verifying line endings, which would not catch a regression in CRLF detection.
- ✅ Strengthened setup config test in `setup_mode.rs`:
  - `format_commands_global_scope` now asserts all 18 config entries from `build_config_entries()`, including previously missing: `difftool.gitgpui.trustExitCode`, `difftool.prompt`, `mergetool.prompt`, `merge.guitool`, `diff.guitool`, `mergetool.guiDefault`, `difftool.guiDefault`, and all GUI tool trustExitCode entries.
- ✅ Replaced bare `.unwrap()` calls in core production code with documented `.expect()`:
  - `text_utils.rs:merge_intervals()`: `result.last_mut().unwrap()` → `.expect("result is non-empty by construction")` with invariant comment.
  - `file_diff.rs:patience_lis()`: `tails.last().unwrap()` → `.expect("tails is non-empty after processing pairs")` with invariant comment.
- ✅ Validation: `cargo test --workspace --no-default-features --features gix` (**1169 passed, 0 failed, 5 ignored**).
- ✅ Validation: `cargo clippy --workspace --no-default-features --features gix -- -D warnings` (**0 warnings**).
...
Conclusion: All components from both design documents remain fully implemented. This iteration hardened test quality by strengthening CRLF marker assertions, expanding setup config coverage to all 18 entries, and replacing bare `.unwrap()` with documented `.expect()` in core algorithms.

### Progress Snapshot (Iteration 34, Merge Extraction Octopus Pagination Hardening — March 2, 2026)

Performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Implemented a Phase 3C robustness hardening in `crates/gitgpui-core/src/merge_extraction.rs`:
  - `discover_merge_commits()` now paginates `git rev-list --merges --parents` output using `--skip`, continuing until it finds the requested number of exactly-two-parent merges or history is exhausted.
  - This fixes a discovery gap where valid two-parent merges could be missed when recent history contains multiple octopus merges.
- ✅ Added regression coverage in `crates/gitgpui-core/src/merge_extraction.rs`:
  - `discover_merge_commits_zero_max_merges_errors`
  - `discovers_merge_commits_after_recent_octopus_merges`
- ✅ Validation: `cargo test -p gitgpui-core merge_extraction -- --nocapture` (**8 passed, 0 failed**).
...
- ✅ Phase 2A–2C: KDiff3-style fixture harness — 16 tests + 9 seed fixtures.
- ✅ Phase 3A–3C: Permutation corpus (243 sampled + 161K on-demand) + real-world merge extraction.
  - Hardening this iteration: merge discovery now paginates past octopus merges to reliably return exactly-two-parent merges.
- ✅ Phase 4A: Mergetool E2E — 65 tests.
- ✅ Phase 4B: Difftool E2E — 32 tests.
- ✅ Phase 5A–5C: Meld-derived algorithm tests — 32 tests.
- 🔧 Partially implemented components: none.
...
Conclusion: All components from both design documents remain fully implemented. This iteration hardened Phase 3C merge discovery semantics for octopus-heavy histories and added dedicated regression tests.
...
Performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Full codebase audit via two independent exploration passes confirming 100% implementation coverage.
- ✅ Verified all 10 behavior matrix items have production implementations and dedicated tests:
  1. File paths with spaces and unicode — tested in mergetool, difftool, dir-diff modes.
  2. Invocation from repo subdirectory — tested in both tool modes plus dir-diff.
  3. No-base conflicts (BASE absent) — tested with empty-base flag and zdiff3 label variants.
  4. Binary and non-UTF8 content — binary merge heuristics, non-UTF-8 byte preservation tested.
  5. Deleted output — trust-exit-code deleted output, delete/delete conflict choices tested.
  6. Symlink conflicts — symlink target resolution, broken symlinks, cycle detection tested.
  7. Submodule path conflicts — local/remote choice, deleted submod, file-vs-submod, dir-vs-submod tested.
  8. CRLF preservation — conflict markers match input line endings, CRLF-to-LF change detection tested.
  9. Directory diff mode — tree copy with symlink dereferencing, pathspec from subdirectory tested.
  10. Close/cancel behavior and exit code — focused GUI launch gating, exit code 0/1/≥2 contract tested.
- ✅ Verified zero `.unwrap()` calls in production code across all 6 app crate source files.
- ✅ Verified zero TODO/FIXME/HACK comments in production code.
- ✅ Verified setup command emits all 18 config entries matching design doc exactly.
- ✅ Validation: `cargo test --workspace --no-default-features --features gix` (**1161 passed, 0 failed, 5 ignored**).
- ✅ Validation: `cargo clippy --workspace --no-default-features --features gix -- -D warnings` (**0 warnings**).
...
Conclusion: All components from both design documents remain fully implemented. This iteration performed a comprehensive two-pass verification audit confirming complete implementation coverage, clean production code quality, and full test suite health (1161 tests, 0 failures, 0 clippy warnings).
...
Implementation performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Comprehensive audit of all design document scenarios confirmed 100% implementation coverage across both documents.
- ✅ Comprehensive production code robustness audit confirmed zero `.unwrap()` calls in production paths.
- ✅ Identified exit code bug: `--help`, `--version`, and subcommand `--help` all exited with code 2 (ERROR) instead of code 0 (SUCCESS), violating the design doc's exit code policy.
- ✅ Root cause: `Cli::try_parse_from()` returns `clap::Error` with `DisplayHelp`/`DisplayVersion` kinds for informational output, which fell through to the compat parser, failed there, and was treated as a real error.
- ✅ Fixed `parse_app_mode_from_args_env_and_config()` in `crates/gitgpui-app/src/cli.rs`:
  - Added early detection of `ErrorKind::DisplayHelp` and `ErrorKind::DisplayVersion` clap errors before compat parsing.
  - These informational errors now call `clap_err.exit()` which prints to stdout and exits with code 0.
- ✅ Added integration tests in `crates/gitgpui-app/tests/standalone_tool_mode_integration.rs`:
  - `help_flag_exits_zero` — verifies `--help` exits 0
  - `version_flag_exits_zero` — verifies `--version` exits 0
  - `subcommand_help_exits_zero` — verifies `difftool --help`, `mergetool --help`, `setup --help` all exit 0
  - `help_subcommand_exits_zero` — verifies `help` subcommand exits 0
- ✅ Validation commands:
  - `cargo test --workspace --no-default-features --features gix` (**1150 passed, 0 failed, 5 ignored**)
  - `cargo clippy --workspace --no-default-features --features gix -- -D warnings` (**0 warnings**)

External Diff/Merge Usage Design (`external_usage.md`):
- ✅ CLI modes: `difftool`, `mergetool`, and `setup` implemented with all documented flags and env fallback.
- ✅ Exit policy: dedicated modes return `0`/`1`/`>=2` per design contract; `--help`/`--version` now correctly exit 0.
- ✅ Git integration: setup/config emits full headless+GUI tool config with `guiDefault=auto`.
- ✅ Compatibility: KDiff3/Meld invocation forms supported (`--L1/--L2/--L3`, `-o/--output/--out`, `--base`, positional forms).
- ✅ Behavior matrix: all 10 required scenarios covered by automated tests.
- 🔧 Partially implemented components: none.
...
Conclusion: All components from both design documents remain fully implemented; this iteration fixed a correctness bug where `--help`/`--version` violated the exit code policy by returning error code 2 instead of success code 0.
...
Implementation performed this iteration:
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Identified parity gap: behavior-matrix item 1 (paths with spaces/unicode) was covered for Git-driven invocations and Unicode standalone mode, but lacked explicit standalone direct-mode space-path coverage.
- ✅ Added standalone direct-mode spaced-path integration tests in `crates/gitgpui-app/tests/standalone_tool_mode_integration.rs`:
  - `standalone_mergetool_handles_paths_with_spaces`
  - `standalone_difftool_handles_paths_with_spaces`
- ✅ Validation commands:
  - `cargo test -p gitgpui-app --no-default-features --features gix --test standalone_tool_mode_integration handles_paths_with_spaces -- --nocapture` (**2 passed, 0 failed**)
  - `cargo test -p gitgpui-app --no-default-features --features gix --test standalone_tool_mode_integration` (**54 passed, 0 failed**)

External Diff/Merge Usage Design (`external_usage.md`):
- ✅ CLI modes: `difftool`, `mergetool`, and `setup` implemented with documented flags/env fallback.
- ✅ Exit policy: dedicated modes return `0`/`1`/`>=2` per design contract.
- ✅ Git integration: setup/config emits full headless+GUI tool config with `guiDefault=auto`.
- ✅ Compatibility invocations: KDiff3/Meld forms supported (`--L1/--L2/--L3`, `-o/--output/--out`, `--base`, positional forms).
- ✅ Behavior matrix: all 10 scenarios covered; standalone direct-mode path-with-spaces parity is now explicitly tested for both diff and merge.
- 🔧 Partially implemented components: none.
- ⬜ Not-yet-started components: none.
...
Conclusion: All components from both design documents remain fully implemented; this iteration hardened standalone direct-mode path-with-spaces parity coverage.
...
- ✅ Compatibility: KDiff3/Meld invocation forms supported (`--L1/--L2/--L3`, `-o/--output/--out`, `--base`, positional forms).
- ✅ Behavior matrix: all 10 required scenarios covered by automated tests.
- ✅ Focused GPUI windows: interactive diff and merge windows wired to `--gui` flag.
- 🔧 Partially implemented components: none.
- ⬜ Not-yet-started components: none.
...
- ✅ Read both design documents in full (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`).
- ✅ Re-audited implementation and test inventories in `gitgpui-app`, `gitgpui-core`, and `gitgpui-ui-gpui` against both design documents.
...
External Diff/Merge Usage Design (`external_usage.md`):
- ✅ CLI modes: `difftool`, `mergetool`, and `setup` implemented with documented flags and env fallback.
- ✅ Exit policy: dedicated modes return `0`/`1`/`>=2` per contract.
- ✅ Git integration: setup/config emits full headless+GUI tool config with `guiDefault=auto`.
- ✅ Compatibility: KDiff3/Meld invocation forms supported (`--L1/--L2/--L3`, `-o/--output/--out`, `--base`, positional forms).
...
Reference Test Portability Plan (`docs/REFERENCE_TEST_PORTABILITY.md`):
- ✅ Phase 1A–1C complete (t6403 core merge algorithm, t6427 zdiff3, conflict label formatting).
...
- ✅ Deep agent-driven audit verified all design document components against implementation:
  ...
  - Exit code policy (0/1/≥2) confirmed in cli.rs constants and tested across all modes.
  - All 10 behavior matrix items verified with specific test functions (spaces/unicode, subdirectory, no-base, binary, deleted output, symlink, submodule, CRLF, dir-diff, cancel/exit).
  - KDiff3/Meld compatibility (--L1/--L2/--L3, -o/--output/--out, --base, positional forms) confirmed present.
  - Setup config emits all 18 entries (headless+GUI tools, guiDefault=auto, trustExitCode, prompt).
  - All 6 reference test phases verified: 1A–1C (46 tests), 2A–2C (21 tests + 9 fixtures), 3A–3C (243+ corpus + 10 extraction), 4A (65 tests), 4B (28 tests), 5A–5C (32 tests).
...
- ✅ Phase 1C: Conflict label formatting — 5 tests.
- ✅ Phase 2A–2C: KDiff3-style fixture harness — 21 tests + 9 seed fixtures.
...
Conclusion: All components from both design documents are fully implemented and verified. This is the ninth independent completion verification (iterations 13, 15, 16, 17, 18, 19, 20, 21, 22). Test count stable at 1128.
...
- ✅ `cargo test --workspace --no-default-features --features gix`: **1128 passed, 0 failed, 5 ignored**.
- ✅ `cargo clippy --workspace --no-default-features --features gix -- -D warnings`: **0 warnings**.
- ✅ Three parallel agent-driven audits verified all design document components against implementation:
  - Difftool audit: all 6 CLI flags, env fallbacks (LOCAL/REMOTE/MERGED/BASE), directory diff handling with symlink dereference/cycle detection, exit code policy (0/1/≥2), and `--tool-help` Git-level discoverability all confirmed present.
  - Mergetool & setup audit: all 9 mergetool flags + KDiff3 aliases (--L1/L2/L3, -o/--output/--out), env fallbacks, all 18 setup config entries (headless+GUI tools, guiDefault=auto, trustExitCode, prompt), and 22 setup-specific tests all confirmed present.
  - Behavior matrix audit: all 10 required scenarios mapped to specific test functions across 3 integration test files (208 integration tests total), with no gaps found.
...
Conclusion: All components from both design documents are fully implemented and verified. This is the eighth independent completion verification (iterations 13, 15, 16, 17, 18, 19, 20, 21). Test count stable at 1128.
...
Verification performed this iteration:
- ✅ Read and cross-checked both design documents (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`) against the current codebase.
- ✅ `cargo test --workspace --no-default-features --features gix`: **1127 passed, 0 failed, 5 ignored**.
- ✅ `cargo clippy --workspace --no-default-features --features gix -- -D warnings`: **0 warnings**.
- ✅ Deep agent-driven audit verified all design document components against implementation:
  - All 10 behavior matrix items (spaces/unicode, subdirectory, no-base, binary, deleted output, symlink, submodule, CRLF, dir-diff, cancel/exit) have automated test coverage.
  - CLI modes (difftool, mergetool, setup) fully implemented with all documented flags and env fallback.
  - Exit code policy (0/1/≥2) correctly implemented.
  - Setup config emits all 18 entries (headless+GUI tools, guiDefault=auto, trustExitCode, prompt).
  - KDiff3/Meld compatibility modes (--L1/--L2/--L3, -o/--output/--out, positional) present.
  - All 5 reference test phases (1A–1C, 2A–2C, 3A–3C, 4A–4B, 5A–5C) fully ported.
...
Conclusion: All components from both design documents are fully implemented and verified. This is the seventh independent completion verification (iterations 13, 15, 16, 17, 18, 19, 20). Test count stable at 1127.
...
Verification performed this iteration:
- ✅ Read and cross-checked both design documents (`external_usage.md`, `docs/REFERENCE_TEST_PORTABILITY.md`) against the current codebase.
- ✅ `cargo test --workspace --no-default-features --features gix`: **1125 passed, 0 failed, 5 ignored**.
- ✅ `cargo clippy --workspace --no-default-features --features gix -- -D warnings`: **0 warnings**.
- ✅ `cargo build --workspace` (full build with UI features): **compiles successfully**.
- ✅ Deep agent-driven audit verified all design document components against implementation:
  - Setup mode emits correct headless+GUI tool config with guiDefault=auto.
  - `--gui` flag on both difftool/mergetool; headless builds return clear error, UI builds launch GPUI windows.
  - All 10 behavior matrix scenarios have automated test coverage.
  - Phase 3C real-world merge extraction fully operational (768-line test module).
  - Phase 5A–5C Meld algorithm parity tests all present (myers blocks, interval merging, newline ops).
  - Submodule conflict handling verified across E2E suite (12+ tests).
  - `--tool-help` discoverability parity confirmed via Git integration tests.
...
Conclusion: All components from both design documents are fully implemented and verified. This is the sixth independent completion verification (iterations 13, 15, 16, 17, 18, 19). Test count stable at 1125.
...
- ✅ `cargo clippy --workspace --no-default-features --features gix -- -D warnings`: **0 warnings**.
- ✅ Zero `TODO`/`FIXME`/`HACK`/`unimplemented!()`/`todo!()` in any first-party production `src/` code (only in vendored gpui/resvg).
- ✅ Deep agent-driven cross-reference verified all CLI flags, exit codes, env fallbacks, setup mode, and test phases against both design documents.
...
Conclusion: All components from both design documents are fully implemented and verified. This is the fifth independent completion verification (iterations 13, 15, 16, 17, 18). Test count stable at 1123.
