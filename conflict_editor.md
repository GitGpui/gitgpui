...
- ✅ `4. Save/Cancel/Exit Semantics` — Added focused-mergetool save/exit plumbing in unified GPUI flow: threaded mergetool labels + shared exit-code handle through `GitGpuiViewConfig`/`MainPaneView`, switched focused resolver Save to write merged output directly using `conflict_resolver::generate_resolved_text_with_options` with `UnresolvedConflictMode::PreserveMarkers` + labels, hid `Save & stage` in focused mode, and enforced mergetool exit contract (`save resolved -> 0`, `save unresolved -> 1`, `escape/close -> 1`, write/setup failures -> `2`). Added unit coverage for labeled focused launch config, preserve-markers generation path, focused save exit-code derivation, and focused-mode save-stage visibility gating. Ran required `cargo test --workspace --no-default-features --features gix` and `cargo clippy --workspace --no-default-features --features gix -- -D warnings`; test/clippy still fail in pre-existing difftool integration and existing clippy issues under `gitgpui-ui-gpui`.
