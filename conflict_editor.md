...
- 🔧 `1. Add Focused Mergetool View Mode to Existing UI` — Added `GitGpuiViewMode` + `GitGpuiViewConfig` in `gitgpui-ui-gpui::view`, introduced `GitGpuiView::new_with_config`, and threaded view mode into `MainPaneView` construction/state as foundational plumbing (focused-mode chrome/action gating still pending).
