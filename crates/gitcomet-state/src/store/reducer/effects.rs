...
pub(super) fn append_ensure_sidebar_data_effects(
    ...
    if request.subtrees && matches!(repo_state.subtrees, Loadable::NotLoaded) {
        repo_state.set_subtrees(Loadable::Loading);
        effects.push_effect(Effect::LoadSubtrees { repo_id });
    }
...
mod tests {
    ...
    fn ensure_sidebar_data_stores_request_before_repo_is_open() {
        ...
        assert!(matches!(repo.subtrees, Loadable::NotLoaded));
    ...
    fn ensure_sidebar_data_loads_only_missing_requested_sections() {
        ...
        assert!(effects.iter().any(
            |effect| matches!(effect, Effect::LoadSubtrees { repo_id: rid } if *rid == repo_id)
        ));
        ...
        assert!(repo.subtrees.is_loading());
