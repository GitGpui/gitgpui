...
fn has_subtree_load_effect(effects: &[Effect], repo_id: RepoId) -> bool {
    effects.iter().any(|effect| {
        matches!(
            effect,
            Effect::LoadSubtrees { repo_id: candidate } if *candidate == repo_id
        )
    })
}
...
fn ensure_sidebar_data_retries_requested_sections_after_repo_opened() {
    ...
    assert!(matches!(state.repos[0].worktrees, Loadable::NotLoaded));
    assert!(matches!(state.repos[0].submodules, Loadable::NotLoaded));
    assert!(matches!(state.repos[0].subtrees, Loadable::NotLoaded));
    assert!(matches!(state.repos[0].stashes, Loadable::NotLoaded));
...
fn set_active_repo_replays_stored_sidebar_data_request() {
    ...
    repo1_state.set_subtrees(Loadable::NotLoaded);
    ...
    assert!(has_subtree_load_effect(&effects, repo1));
    ...
    assert!(repo1_state.subtrees.is_loading());
