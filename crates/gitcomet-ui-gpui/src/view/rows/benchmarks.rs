...
#[derive(Clone)]
struct CommitFileRowPresentationCache<K> {
...
impl<K: Clone + Eq + Hash> CommitFileRowPresentationCache<K> {
    fn rows_for(&mut self, key: &K, files: &[CommitFileChange]) -> u64 {
...
fn commit_file_kind_key(kind: FileStatusKind) -> u8 {
    match kind {
...
fn empty_history_graph_heads<'a>() -> HashSet<&'a str> {
    HashSet::default()
}

fn history_graph_heads_from_indices<'a>(
    commits: &'a [Commit],
...
fn history_graph_heads_from_branches<'a>(
    branches: &'a [Branch],
    remote_branches: &'a [RemoteBranch],
) -> HashSet<&'a str> {
    branches
...
#[derive(Clone, Copy)]
struct CommitFileKindVisuals {
    kind_key: u8,
}

fn commit_file_kind_visuals(kind: FileStatusKind) -> CommitFileKindVisuals {
