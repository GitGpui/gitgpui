...
impl XDPEventSource {
    pub fn new(executor: &BackgroundExecutor) -> Self {
        ...
        super::tokio_runtime().spawn(async move {
            if let Err(err) = async {
                    ...
                    .await
                {
                    ...
                    let _cursor_theme_task = super::tokio_runtime().spawn(async move {
                ...
                {
                    let sender = sender.clone();
                    let _cursor_size_task = super::tokio_runtime().spawn(async move {
            ...
            .await
            {
                log::error!("XDG portal settings task failed: {err}");
            }
        });
