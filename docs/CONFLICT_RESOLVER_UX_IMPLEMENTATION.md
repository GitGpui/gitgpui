...
- ✅ `view/rows/conflict_resolver.rs` + `view/panes/main.rs` + `view/panels/{mod.rs,popover.rs,popover/context_menu.rs,popover/context_menu/conflict_resolver_input_row.rs}` + `view/conflict_resolver.rs` + `view/mod.rs` — added resolver input-row right-click context menu with `"Select line (N)"` / `"Select chunk (Ln X - Y)"` actions for three-way and two-way rows; labels are derived from displayed gutter line numbers and chunk ranges; actions dispatch via new `ResolverPickTarget` through popover context-menu plumbing.
